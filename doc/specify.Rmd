...
These quantities can be invoked by the more general function `eval_representation()`.
...
`eval_representaton()` is a useful function for accessing quantities that are not built-in to `distionary`, like `g` and `another_representation` in this example.
