...
range. They are properties like any other --- both are determined by the
distribution, and either could in principle be worked out from a CDF, by
finding where it jumps and where it stops moving. What they cannot be is
*stated by name*, because doing that reliably from a CDF is a numerical
problem rather than a mathematical one. So the distribution declares its
support instead, and both are read off that.
