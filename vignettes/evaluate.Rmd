...
A _distributional representation_ is a function that fully defines the distribution, such that any property can be calculated from it. Here is a list of representations recognised by `distionary`, and the functions for accessing them. 
...
All representations can either be accessed by the `eval_*()` family of functions, or `enframe_*()`. While `eval_*()` returns a vector of the evaluated representation, `enframe_*()` returns the results in a tibble or data frame paired with the inputs. 

The `enframe_*()` functions allow for insertion of multiple distributions, placing a column for each distribution. For example, here is the evaluated PMF.
...
The column names can be changed in three ways:
