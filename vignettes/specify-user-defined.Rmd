...
- `.support`, as seen in the example starting this vignette, specifies the distribution's support -- the set on which it places probability -- using `continuous()`, `discrete()`, or `mixed()`. The variable type (continuous, discrete, or mixed) and the range are *derived* from it. (The older `.vtype` argument, which took a string such as `"continuous"`, is superseded by `.support`.)
