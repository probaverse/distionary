...
|----|------------|------|
| 1. | Numerically computed moments of a distribution with infinitely many atoms can be wrong, without saying so, if the probability thins out and then picks up again. | The atoms are summed by walking outward through the support (via the `discretes` package), stopping once a run of them carries negligible probability and contributes negligibly. That is a reading of the atoms just visited, not a bound on the ones past them, so a distribution that goes quiet and then resumes is cut off early. A sum that never goes quiet returns `NaN` rather than a wrong answer. |
