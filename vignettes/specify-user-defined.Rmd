...
Some representations come in more than one variant, described in the [Evaluate a Distribution](./evaluate.html) vignette: the quantile function has a left and a right inverse, the CDF can count the point it is evaluated at or leave it out, and so on.
...
|----|------------|------|
| 1. | Numerically computed moments of a distribution with infinitely many atoms rely on the tail of the sum being negligible, and return `NaN` if it does not converge. | The atoms are summed by walking outward through the support (via the `discretes` package) until contributions fall below tolerance; a non-convergent sum, such as for a heavy-tailed distribution, cannot be detected exactly. |
