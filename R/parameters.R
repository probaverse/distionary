...
# TO DO: define parameters(distribution) <- ...
...
#' Ignores whether parameters are already resolved.
#' TO DO: :is such a function useful as user-facing?
validate_parameters_space_only <- function(distribution, ..., .stop = FALSE) {
  TRUE
}

set_parameters <- function(distribution, ...) {
  dots <- rlang::list2(...) # To do: flatten lists to accept list entries.
  ## To named entries, resolve parameters:
  already_set <- parameters(distribution)
  not_already_set <- vapply(already_set, is.null, FUN.VALUE = logical(1L))
  param_values <- append(dots, already_set[!not_already_set])
  distribution[["parameters"]][["values"]] <- param_values
  ## TO DO: replace above line with parameters(distribution) <- param_values
  ## TO DO: validate parameters; setting is not allowed unless valid.
  ## To unnamed entries, add to space:
  ## <TO DO>
