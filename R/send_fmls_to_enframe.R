...
#' @param eval_fn String; suffix of an `eval_` function, specifying the
#' distributional representation to be evaluated.
#' @return Either the `enframe_uni()`, `enframe_bi()`, or `enframe_multi()`
#' function (depending on the `variate` input), evaluated at the
#' encapsulating function's arguments, with the `eval_fn` argument
#' also passed.
send_fmls_to_enframe <- function(variate, eval_fn) {
